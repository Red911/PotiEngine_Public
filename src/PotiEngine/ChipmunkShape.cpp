...
namespace PotiEngine
{
	...
	cpCollisionType ChipmunkShape::GetType(const ChipmunkShape& shape)
	{
		return cpShapeGetCollisionType(shape.m_handle);
	}

	void ChipmunkShape::SetType(const ChipmunkShape& shape, cpCollisionType value)
	{
		cpShapeSetCollisionType(shape.m_handle, value);
	}
	...
	ChipmunkShape ChipmunkShape::BuildConvexHull(ChipmunkBody& body, const std::vector<Vector2f>& positions, const Matricef& transform, float radius)
	{
		std::vector<cpVect> verts;
		verts.reserve(positions.size());
		for (std::size_t i = 0; i < positions.size(); ++i)
		{
			Vector2f pos = Vector2((transform * positions[i]).GetMatrice()[0], (transform * positions[i]).GetMatrice()[1]);
	...
	float ChipmunkShape::ComputePolyMoment(float mass, const std::vector<Vector2f>& positions, const Matricef& transform, float radius)
	{
		std::vector<cpVect> verts;
		verts.reserve(positions.size());
		for (std::size_t i = 0; i < positions.size(); ++i)
