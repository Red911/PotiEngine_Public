...
namespace PotiEngine
{
	...
	void ChipmunkSpace::DebugDraw(Renderer& renderer, const Matricef& cameraInverseTransform)
	{
		struct DrawData
		{
			Renderer& renderer;
			const Matricef& viewMatrix;
		};
		...
		drawOptions.drawCircle = [](cpVect pos, cpFloat /*angle*/, cpFloat radius, cpSpaceDebugColor outlineColor, cpSpaceDebugColor /*fillColor*/, cpDataPointer data)
			{
				...
				for (std::size_t i = 0; i < 20; ++i)
				{
					...
					Vector2f finalPos = Vector2f((drawData.viewMatrix * Vector2f(pos.x + std::sin(angle) * radius, pos.y + std::cos(angle) * radius))[0], (drawData.viewMatrix * Vector2f(pos.x + std::sin(angle) * radius, pos.y + std::cos(angle) * radius))[1]);
					points[i] = { finalPos.x, finalPos.y };
				}
				// On ferme le cercle
				points[20] = points[0];

				drawData.renderer.DrawColor(outlineColor.r * 255, outlineColor.g * 255, outlineColor.b * 255, outlineColor.a * 255);
		...
		drawOptions.drawDot = [](cpFloat size, cpVect pos, cpSpaceDebugColor color, cpDataPointer data)
			{
				...
				Vector2f dotPos = Vector2f((drawData.viewMatrix * Vector2f(static_cast<float>(pos.x), static_cast<float>(pos.y)))[0], (drawData.viewMatrix * Vector2f(static_cast<float>(pos.x), static_cast<float>(pos.y)))[1]);
				...
				drawData.renderer.DrawColor(color.r * 255, color.g * 255, color.b * 255, color.a * 255);
				drawData.renderer.RenderLines(points.data(), points.size());
			};
		...
		drawOptions.drawFatSegment = [](cpVect a, cpVect b, cpFloat radius, cpSpaceDebugColor outlineColor, cpSpaceDebugColor /*fillColor*/, cpDataPointer data)
			{
				...
				Vector2f from = Vector2f((drawData.viewMatrix * Vector2f(static_cast<float>(a.x), static_cast<float>(a.y)))[0], (drawData.viewMatrix * Vector2f(static_cast<float>(a.x), static_cast<float>(a.y)))[1]);
				Vector2f to = Vector2f((drawData.viewMatrix * Vector2f(static_cast<float>(b.x), static_cast<float>(b.y)))[0], (drawData.viewMatrix * Vector2f(static_cast<float>(b.x), static_cast<float>(b.y)))[1]);
		...
		drawOptions.drawPolygon = [](int vertexCount, const cpVect* vertices, cpFloat /*radius*/, cpSpaceDebugColor outlineColor, cpSpaceDebugColor /*fillColor*/, cpDataPointer data)
			{
				...
				{
					Vector2f pos = Vector2f((drawData.viewMatrix * Vector2f(static_cast<float>(vertices[i].x), static_cast<float>(vertices[i].y)))[0], (drawData.viewMatrix * Vector2f(static_cast<float>(vertices[i].x), static_cast<float>(vertices[i].y)))[1]);
