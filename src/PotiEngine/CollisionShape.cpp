...
namespace PotiEngine
{
	...
	ConvexShape::ConvexShape(const Model& model, Matricef transformation, float radius_) :
		radius(radius_)
	{
		const std::vector<ModelData>& vertices = model.GetVertices();
	...
	{
		ChipmunkShape shape = ChipmunkShape::BuildConvexHull(body, positions, Matricef::Translate(offset));
		shape.SetFriction(friction);
	...
	{
		return ChipmunkShape::ComputePolyMoment(mass, positions, Matricef::Translate(offset));
	}
