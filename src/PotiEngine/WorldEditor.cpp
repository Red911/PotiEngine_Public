...
namespace PotiEngine
{
	...
	{
		PotiEngine::Transform* targetTransform = m_registry.try_get<PotiEngine::Transform>(entity);
		if (!targetTransform)
		{
		...
		PotiEngine::Transform& cameraTransform = m_registry.get<PotiEngine::Transform>(camera);
	...
	void WorldEditor::DisplayEntityList()
	{
		for (auto [entity] : m_registry.storage<entt::entity>().each())
		{
			...
			if (PotiEngine::Transform* transform = m_registry.try_get<PotiEngine::Transform>(entity))
			{
	...
	void WorldEditor::DisplayEntityListNode(entt::entity entity)
	{
		...
		if (PotiEngine::NameComponent* nameComponent = m_registry.try_get<PotiEngine::NameComponent>(entity))
		{
